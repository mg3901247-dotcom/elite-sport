document.addEventListener("DOMContentLoaded", () => {
    ...
    /*=========================================
        ⏱️ TIME GREETING + LIVE CLOCK + 🎆 SKY SHOW
        (الساعة + الشهب + الصواريخ + النجوم)
    =========================================*/
    ...
    /*=========================================
            FORM VALIDATION  (الكود الأصلي بتاعك)
    =========================================*/
    ...
    // ⚙️ نفس مفتاح Web3Forms المستخدم في التسجيل (script.js → CONFIG.WEBSITE.web3formsAccessKey).
    // لو الملف ده بيشتغل بدون script.js، حط المفتاح هنا مباشرة.
    const WEB3FORMS_ACCESS_KEY =
        (typeof CONFIG !== "undefined" && CONFIG.WEBSITE && CONFIG.WEBSITE.web3formsAccessKey) ||
        "YOUR_WEB3FORMS_ACCESS_KEY";
    const WEB3FORMS_ENDPOINT = "https://api.web3forms.com/submit";
    const CONTACT_EMAIL =
        (typeof CONFIG !== "undefined" && CONFIG.WEBSITE && CONFIG.WEBSITE.email) || "";

    function mailtoFallback(data) {
        const subject = encodeURIComponent("رسالة تواصل جديدة");
    ...
    form.addEventListener(
        ...
        async function (event) {
            ...
            try {
                ...
                console.error("[Contact] Web3Forms submission failed:", err);
                mailtoFallback(data);
            } finally {
                if (button) { button.disabled = false; button.innerHTML = originalHtml; }
            }
